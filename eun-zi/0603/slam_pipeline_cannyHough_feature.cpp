...
int main() {
    ...
    HoughLinesP(canny_edges, lines, 1, CV_PI / 180, 40, 30, 10);
    Canny(img_right, canny_edges_right, 50, 100);
    HoughLinesP(canny_edges_right, lines_right, 1, CV_PI / 180, 40, 30, 10);
    ...
    //orb->detectAndCompute(img_left, hough_mask, kp_left_raw, desc_left_raw); 기존 소스코드
    //orb->detectAndCompute(img_right, hough_mask_right, kp_right_raw, desc_right_raw);
    
    
    // 추가 선분 끝점만 KeyPoint로 변환
	vector<Point2f> pts_l, pts_r;
		for (auto& ln : lines) {
			float dx = ln[2] - ln[0];
			float dy = ln[3] - ln[1];
			float length = sqrt(dx * dx + dy * dy);
			if (length < 30.0f) continue;  // 너무 짧은 선 무시

			pts_l.emplace_back(ln[0], ln[1]);
			pts_l.emplace_back(ln[2], ln[3]);
	}
		for (auto& ln : lines_right) {
			float dx = ln[2] - ln[0];
			float dy = ln[3] - ln[1];
			float length = sqrt(dx * dx + dy * dy);
			if (length < 30.0f) continue;
			
			pts_r.emplace_back(ln[0], ln[1]);
			pts_r.emplace_back(ln[2], ln[3]);
		}
	kp_left_raw.clear();
	kp_right_raw.clear();
	for (auto& p : pts_l) kp_left_raw.emplace_back(p, 1.f);
	for (auto& p : pts_r) kp_right_raw.emplace_back(p, 1.f);

	// ORB 디스크립터만 생성
	orb->compute(img_left, kp_left_raw, desc_left_raw);
	orb->compute(img_right, kp_right_raw, desc_right_raw);
    ...
    std::cout << "매칭된 선 수: " << valid_matches.size() << std::endl;
    ...
    imwrite("/home/eunzi/Desktop/direction_img.jpg", direction_img);
